...
const FarmCards: React.FC = () => {
  ...
  const hotsIndex = farms.findIndex(
    ({ tokenSymbol }) => tokenSymbol === 'HOTS',
  )

  const hotsPrice =
    hotsIndex >= 0 && stakedValue[hotsIndex]
      ? stakedValue[hotsIndex].tokenPriceInWbnb
      : new BigNumber(0)

  const BLOCKS_PER_YEAR = new BigNumber(2336000)
  const HOTS_PER_BLOCK = new BigNumber(1)
...
const FarmCard: React.FC<FarmCardProps> = ({ farm }) => {
  ...
  }, [hots, lpTokenAddress, account, setHarvestable])

  const poolActive = true // startTime * 1000 - Date.now() <= 0

  return (
    <StyledCardWrapper>
      {farm.tokenSymbol === 'HOTS' && <StyledCardAccent />}
      <Card>
        <CardContent>
          <StyledContent>
          <StyledTopIcon>
          ...
          <StyledInsight><span><b>Deposit</b></span> <span>{farm.name}</span></StyledInsight>
            <StyledInsight>
              ...
              <span>
               <img src={Hots} height="12" /> {farm.rewardValue}
              </span>
            </StyledInsight>
            <StyledInsight>
              <span><b>APY</b></span>
